...
def test_admin_logout(login, page: Page, admin_page: AdminPage):
...
def test_add_new_category(login, page: Page, admin_page: AdminPage):
    ...
    admin_page.fill_new_category_general_tab(category_name="new_category", category_tag="new_category_tag")
    admin_page.open_tab(CatalogTabs.Data)
    admin_page.fill_new_category_data_tab(sort_order="new_category_sort_order")
    admin_page.open_tab(CatalogTabs.SEO)
    admin_page.fill_new_category_seo_tab(key_word="new_category_key_word")
    admin_page.open_tab(CatalogTabs.Design)
    admin_page.save_new_category()
    notification = page.locator(success_notification)
    category_created = notification.text_content()
    assert category_created == success_added_category
...
def test_add_new_product(login, page: Page, admin_page: AdminPage):
    ...
    admin_page.fill_new_product_general_tab(product_name="new_product", product_tag="new_product")
    admin_page.open_tab(CatalogTabs.Data)
    admin_page.fill_new_product_data_tab(model="new_product")
    admin_page.open_tab(CatalogTabs.SEO)
    admin_page.fill_new_product_seo_tab(key_word="new_product")
